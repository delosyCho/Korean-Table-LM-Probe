...
class KoNET:
    def __init__(self, firstTraining, testCase=False):
        ...
        self.save_path = '/home/ai/pycharm_project/adv_bert/model.ckpt'
        #self.save_path = 'E:\\bert_adv\\bert_model.ckpt'
        self.bert_path = 'E:\\robert_base\\roberta_base.ckpt'
        ...
        self.vocab = tokenization.load_vocab("html_mecab_vocab_128000.txt")
        self.tokenizer = tokenization.WordpieceTokenizer(vocab=self.vocab)
        self.f_tokenizer = tokenization.FullTokenizer(vocab_file="html_mecab_vocab_128000.txt")
    ...
    def get_adv_output(self, bert_config, input_tensor, labels_, dis_lambda, global_step):
        """Get loss and log probs for the next sentence prediction."""

        # Simple binary classification. Note that 0 is "next sentence" and 1 is
        # "random sentence". This weight matrix is not used after pre-training.
        with tf.variable_scope("cls/seq_discrimination"):
            ...
            labels = tf.ones_like(labels_, dtype=tf.float32)

            logits = tf.matmul(input_tensor, output_weights, transpose_b=True)
            logits = tf.nn.bias_add(logits, output_bias)
            log_probs = tf.nn.softmax(logits, axis=-1)

            loss = tf.reduce_mean(kl(tf.cast(log_probs, dtype=tf.float32), labels)
                                  * tf.cast(dis_lambda, dtype=tf.float32))
            return loss, log_probs, log_probs

    def get_discrimination_output(self, bert_config, input_tensor, labels):
        """Get loss and log probs for the next sentence prediction."""

        # Simple binary classification. Note that 0 is "next sentence" and 1 is
        # "random sentence". This weight matrix is not used after pre-training.
        with tf.variable_scope("cls/seq_discrimination", reuse=True):
            output_weights = tf.get_variable(
                "output_weights",
                shape=[2, bert_config.hidden_size],
                initializer=modeling.create_initializer(bert_config.initializer_range))
    ...
    def get_vocab(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            vocab_embedding = model.get_embedding_table()
            ...
            vocab_output = sess.run(vocab_embedding, feed_dict=feed_dict)

        vocab_output = np.array(vocab_output, dtype=np.float32)
        print('save vocab embedding', vocab_output.shape)
        np.save("roberta_vocab_embedding", vocab_output)
    ...
    def Training(self, is_Continue, training_epoch):
        ...
        with tf.Session(config=config) as sess:
            ...
            loss2, _, _ = self.get_adv_output(bert_config, input_tensor, domain_label,
                                                  dis_lambda=0.5,
                                                  global_step=global_step)
            loss3, _, _ = self.get_discrimination_output(bert_config, input_tensor, domain_label)

            model_loss = loss1 + loss2
            disc_loss = loss3
            ...
            disc_vars = get_variables_with_name('cls/seq_discrimination')

            optimizer_model = optimization.create_optimizer(loss=model_loss, init_lr=5e-5, num_train_steps=125000,
                                                      num_warmup_steps=5000, use_tpu=False, var_list=output_vars,
                                                            global_step=global_step)

            optimizer_disc = optimization.create_optimizer(loss=disc_loss, init_lr=5e-5, num_train_steps=125000,
            ...
            for i in range(training_epoch):
                ...
                label_ids, label_weight, label_position, domain_label = self.processor.next_batch_adv()
                ...
                g1, loss_, _ = sess.run([global_step, model_loss, optimizer_model], feed_dict=feed_dict)
                try:
                    g2, loss_2, _ = sess.run([global_step, disc_loss, optimizer_disc], feed_dict=feed_dict)
                except:
                    g2 = g1
                    loss_2 = 0

                print(g1, i, np.array(loss_).shape)
    ...
    def eval_LAMA_table(self):
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True
        config.gpu_options.per_process_gpu_memory_fraction = 0.95

        bert_config = modeling.BertConfig.from_json_file('bert_config_mecab_base.json')
        statement_file = open('triples_for_koelectra', 'r', encoding='utf-8')
        ...
        with tf.Session(config=config) as sess:
            ...
            lines = statement_file.read().replace('국가', ' 국가').replace('언어', ' 언어').split('\n')
            lines.pop(-1)
            ...
            for line in lines:
                ...
                for i in range(1000):
                    ...
                    if len(self.f_tokenizer.inv_vocab[arg_max[i]]) == 1:
                        continue
                    if self.f_tokenizer.inv_vocab[arg_max[i]] == '비고':
                        continue
                    if self.f_tokenizer.inv_vocab[arg_max[i]] == '이름':
                        continue
                    """
                    c += 1
    ...
    def eval_LAMA2(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(157):
                ...
                print('count:', count, 'avg rr:', total_rr / count)
                for i in range(6):
                    print('유형' + str(i) + ' , ', type_rr[i] / type_cnt[i])
                print('------')